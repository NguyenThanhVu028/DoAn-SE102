...
void CNormalPiranhaPlant::Render() {
	if(type == NORMAL_PIRANHA_PLANT_GREEN) CAnimations::GetInstance()->Get(NORMAL_PIRANHA_GREEN_ANIMATION_RISING)->Render(x, y);
	else if (type == NORMAL_PIRANHA_PLANT_RED) CAnimations::GetInstance()->Get(NORMAL_PIRANHA_RED_ANIMATION_RISING)->Render(x, y);
}
